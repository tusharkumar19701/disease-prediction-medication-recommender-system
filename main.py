...
def helper(disease):
    ...
    prec = precautions[precautions['Disease'] == disease][
    ...
    wrkt = workout[workout['disease'] == disease]['workout']

    return desc, prec, med, diet, wrkt;
...
def get_predicted_value(user_symptoms):
    arr = np.zeros(len(symptoms_list))

    for item in user_symptoms:
        arr[symptoms_list[item]] = 1
...
def predict():
    if request.method == 'POST':
        ...
        desc, prec, med, diet, wrkt = helper(predicted_disease)
        prec = np.array(prec)
        prec = prec.tolist()

        return render_template('index.html',predicted_disease=predicted_disease,dis_desc=desc,dis_prec=prec[0],dis_diet=diet,dis_wrkt=wrkt,dis_med=med)
